...
import { createBlackboard } from './core/ai/blackboard';
...
function demoWorld(): World {
  ...
  const entries: Array<
    [keyof typeof FRAMES, number, number, 'hero' | 'monster' | 'building', 'crown' | 'monsters']
  > = [
    ['palace_intact', 46, 49, 'building', 'crown'],
    ['warrior_idle_s_00', 42, 47, 'hero', 'crown'],
    ['ranger_idle_s_00', 48, 45, 'hero', 'crown'],
    ['rogue_idle_s_00', 45, 52, 'hero', 'crown'],
    ['ratkin_idle_s_00', 56, 42, 'monster', 'monsters'],
    ['goblin_idle_s_00', 62, 39, 'monster', 'monsters'],
  ];
  for (const [frame, x, y, kind, faction] of entries) {
    const entity = world.spawn({ kind, faction, x, y });
    entity.renderable = { frame: FRAMES[frame], tint: 0xffffff, prevX: x, prevY: y };
    if (kind === 'hero')
      entity.agent = {
        entity: entity.handle,
        classId: frame.startsWith('warrior')
          ? 'warrior'
          : frame.startsWith('ranger')
            ? 'ranger'
            : 'rogue',
        name: frame.split('_')[0] as string,
        traits: { greed: 1, courage: 1, curiosity: 1, loyalty: 1 },
        currentGoal: null,
        goalScores: [],
        plan: null,
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      stopControls = attachCameraControls(element, camera, () =>
        renderer?.draw(world.current.snapshot(), 0, texture),
      );
      ...
      const loop = (now: number): void => {
        ...
        renderer?.draw(world.current.snapshot(), accumulator / TICK_MS, texture);
