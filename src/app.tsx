...
export function App(): JSX.Element {
  const canvas = useRef<HTMLCanvasElement>(null); const world = useRef(demoWorld()); const [snapshot, setSnapshot] = useState<Snapshot>(() => world.current.snapshot()); const [selected, setSelected] = useState<EntityId | null>(null);
  useEffect(() => { let disposed = false; let renderer: StageRenderer | null = null; let stopControls = () => {}; let frame = 0; let last = performance.now(); let accumulator = 0;
    void (async () => { const element = canvas.current; if (element === null) return; const sheet = await Assets.load<Spritesheet>('/atlas/game.json'); if (disposed) return; const camera = new Camera({ minX: -3072, minY: 0, maxX: 3072, maxY: 3072 }, innerWidth, innerHeight); renderer = await StageRenderer.create({ canvas: element, camera, width: innerWidth, height: innerHeight }); const texture = (index: number): Texture => sheet.textures[FRAME_NAMES[index] as string] as Texture; stopControls = attachCameraControls(element, camera, () => renderer?.draw(world.current.snapshot(), 0, texture)); const loop = (now: number): void => { accumulator = Math.min(accumulator + now - last, TICK_MS * 5); last = now; while (accumulator >= TICK_MS) { world.current.step(); accumulator -= TICK_MS; setSnapshot(world.current.snapshot()); } renderer?.draw(world.current.snapshot(), accumulator / TICK_MS, texture); frame = requestAnimationFrame(loop); }; frame = requestAnimationFrame(loop); })(); return () => { disposed = true; cancelAnimationFrame(frame); stopControls(); renderer?.destroy(); }; }, []);
  return <main className="rs-game"><canvas ref={canvas} onClick={() => setSelected(world.current.snapshot().entities.find((e) => e.kind === 'hero')?.id ?? null)} /><Hud snapshot={snapshot} onCommand={(command) => world.current.issue(command)} /><HeroInspector snapshot={snapshot} selectedHeroId={selected} onDismiss={() => setSelected(null)} /></main>;
}
