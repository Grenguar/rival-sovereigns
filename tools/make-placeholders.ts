...
const colors = { grass: [90, 139, 74], forest: [67, 107, 57], water: [58, 107, 145], rock: [142, 142, 150], road: [138, 111, 74], warrior: [196, 69, 58], ranger: [63, 158, 92], rogue: [139, 95, 191], ratkin: [122, 106, 74], goblin: [104, 129, 56], goblinRaider: [78, 104, 40], peasant: [176, 156, 122], taxCollector: [198, 170, 78], guard: [92, 108, 158], gold: [232, 185, 60] } as const;
...
const subjects = ['warrior', 'ranger', 'rogue', 'ratkin', 'goblin', 'goblinRaider', 'peasant', 'taxCollector', 'guard'];
...
async function main(): Promise<void> {
  ...
  for (const lair of lairs) for (const state of ['intact', 'damaged', 'rubble']) await write(`${lair}_${state}`, 128, 112, (p) => { diamond(p, 128, 64, colors.rock); const c = lair === 'goblinCamp' ? colors.goblin : colors.ratkin; const height = state === 'rubble' ? 10 : state === 'damaged' ? 30 : 46; for (let y = 56 - height; y < 56; y++) for (let x = 40; x < 88; x++) if (Math.abs(x - 64) < (56 - y) * 0.9) pixel(p, 128, x, y, state === 'damaged' && y % 6 < 2 ? colors.rock : c); });
